...
class high_fre_attntion(nn.Module):
    def __init__(self, dim, group_head, dim_head, kernel_size=5, qkv_bias=False, attn_drop=0):
        super().__init__()
        self.to_qkv = nn.Conv2d(dim, 3*group_head * dim_head, 1, 1, 0, bias=qkv_bias)
        self.convs = nn.Conv2d(3*dim_head*group_head, 3*dim_head*group_head, kernel_size, 1, kernel_size//2, groups=3*dim_head*group_head)
        self.act_blocks = AttnMap(dim_head*group_head)
        self.scalor = dim_head ** -0.5
        self.attn_drop = nn.Dropout(attn_drop)


    def forward(self, x):
        b, c, h, w = x.size()
        qkv = self.to_qkv(x)  # (b (3 m d) h w)
        qkv = self.convs(qkv).reshape(b, 3, -1, h, w).transpose(0, 1).contiguous()  # (3 b (m d) h w)
        q, k, v = qkv  # (b (m d) h w)
        attn = self.act_blocks(q.mul(k)).mul(self.scalor)
...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads,
                 ...
                 drop=0., attn_drop=0., rg_idx=0, b_idx=0, window_size= 8):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.split_size = split_size
        ...
        self.hft = high_fre_attntion(dim, num_heads, dim // num_heads, kernel_size=5, qkv_bias=False, attn_drop=0)
        ...
        # self.lft = low_fre_attention(dim, dim // num_heads, window_size)
...
class DATB(nn.Module):
    def __init__(self, dim, num_heads, reso=64, split_size=[2, 4], shift_size=[1, 2], expansion_factor=4.,
        ...
        self.ffn = GFeedForward(dim, ffn_expansion_factor=1, bias=False)
...
class Mona(nn.Module):
    def __init__(self, in_dim):
        ...
        nn.Conv2d(in_dim // 2, in_dim, kernel_size=1, stride=1),
        nn.BatchNorm2d(in_dim),
        nn.GELU(),
        )

        self.adapter_conv = MonaOp(in_dim//2)
    ...
    def forward(self, x):
        ...
        project2 = self.project2(project1)
...
class GFeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        xl = self.down(x2)
        x2 = x2 - F.interpolate(xl, size=x.size()[-2:], mode='bilinear', align_corners=True)
        x = F.gelu(x2) * x1
...
class DAT(nn.Module):
    ...
    def __init__(self,
        ...
        if resi_connection == '1conv':
            ...
            # self.conv_after_body = Shift8(embed_dim // 8)
        elif resi_connection == '3conv':
